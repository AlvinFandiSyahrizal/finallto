...
<body style="background: lightgray">
    ...
    <div class="container mt-5">
        <div class="row">
            <div class="col-md-12">
                <div>
                    <h3 class="text-center my-4">Line3</h3>
                ...
                <div class="card border-0 shadow-sm rounded">
                    <div class="card-body">
                        <table id="line3Table" class="table table-bordered">
                            <thead>
                <tr>
                    <th>Part Number</th>
                    <th>Assy</th>
            ...
            <tbody>
                @foreach($line3s as $line3)
                    <tr>
                        <td>{{ $line3->PartNumber }}</td>
                        <td>{{ $line3->Assy }}</td>
                        <td>{{ $line3->FlangeNon }}</td>
                        <td>{{ $line3->Wclutch }}</td>


                        <td class="text-center">
                            <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('line3s.destroy', $line3->id) }}" method="POST">
                                <a href="{{ route('line3s.edit', $line3->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                            <form action="{{ route('line3s.destroy', $line3->id) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
...
<script>
    function exportToCSV() {
        const table = document.querySelector('#line3Table'); // Gunakan # untuk mencari id
        ...
        link.setAttribute('download', 'line3_data.csv');
        link.click();
    }
</script>
